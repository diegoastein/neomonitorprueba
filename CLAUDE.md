...
- **v1.07**: Periodic heartbeat (every 10s) using non-write reads `.get()` on both Monitor and Control. Watchdog now resets on heartbeat success, decoupling it from data changes. Fully resolves false disconnect on idle ✓
